...
func InitLedger() (string, error) {
	...
	defer conn.Close()
	defer gw.Close()
	submitResult, commit, err := contract.SubmitAsync("InitLedger")
	if err != nil {
...
func CreateCreditFile(
	id string,
	owner string,
	academy string,
	disciplinarySituation string,
	gradePoint string,
	awardSituation string) (string, error) {
...
func UpdateCreditFile(
	...
	defer conn.Close()
	defer gw.Close()
	submitResult, commit, err := contract.SubmitAsync("UpdateCreditFile", client.WithArguments(id, owner, academy, disciplinarySituation, gradePoint, awardSituation))
	if err != nil {
		return "", fmt.Errorf("failed to submit transaction asynchronously: %w", err)
	}
	fmt.Printf("*** Successfully submitted transaction :%v", string(submitResult))
...
func DeleteCreditFile(id string) (string, error) {
	contract, conn, gw := GetContract()
	defer conn.Close()
	defer gw.Close()
